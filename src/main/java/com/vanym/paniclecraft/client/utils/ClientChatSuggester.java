...
public class ClientChatSuggester {
    ...
    protected void chatRenderTick(TickEvent.RenderTickEvent event) {
        ...
        String text = chat.inputField.getText();
        if (!text.startsWith("/")) {
            this.lastApplied.remove(chat);
            return;
        }
        if (chat.inputField.getText().equals(this.lastApplied.get(chat))) {
            return;
        }
        this.lastApplied.put(chat, chat.inputField.getText());
    ...
    protected static void applySuggestions(
        ...
        String fullinput = chat.inputField.getText();
        ...
        if (fullmatch || !(parent instanceof RootCommandNode)) {
            ...
            int screenX = chat.inputField.func_195611_j(suggContext.startPos);
            ...
            for (Map.Entry<CommandNode<CommandSource>, String> entry : map.entrySet()) {
                ...
                chat.commandUsageWidth = Math.max(chat.commandUsageWidth,
                                                  chat.font.getStringWidth(entry.getValue()));
    ...
    protected static void addSuggestion(ChatScreen chat, Suggestion sugg) {
        ...
        int width = chat.font.getStringWidth(sugg.getText());
        if (width > sugsList.field_198505_b.getWidth()) {
            sugsList.field_198505_b = new Rectangle2d(
                    sugsList.field_198505_b.getX(),
                    sugsList.field_198505_b.getY(),
                    width,
                    sugsList.field_198505_b.getHeight());
        }
        if (sugsList.suggestions.getList().size() <= 10) {
            sugsList.field_198505_b = new Rectangle2d(
                    sugsList.field_198505_b.getX(),
                    sugsList.field_198505_b.getY() - 12,
                    sugsList.field_198505_b.getWidth(),
                    sugsList.field_198505_b.getHeight() + 12);
        }
        if (index <= sugsList.field_198508_e) {
            sugsList.cycle(1);
        }
    }
    ...
    protected static void setSuggestions(ChatScreen chat, Suggestions suggs) {
        ...
        int textWidth = suggs.getList()
                             .stream()
                             .map(Suggestion::getText)
                             .map(chat.font::getStringWidth)
                             .max(Integer::compare)
                             .orElse(0);
        int width = chat.inputField.func_195611_j(suggs.getRange().getStart());
