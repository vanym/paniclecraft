...
public class GuiChess extends GuiScreen {
    ...
    protected void sendMove(ChessGame.Move move) {
        Core.instance.network.sendToServer(new MessageChessMove(this.chessdesk, move));
    }
    ...
    public void updateScreen() {
        if ((this.chessdesk.getWorldObj()
                           .getTileEntity(this.chessdesk.xCoord, this.chessdesk.yCoord,
                                          this.chessdesk.zCoord) == null)
            || this.mc.thePlayer.getDistanceSq(this.chessdesk.xCoord + 0.5D,
                                               this.chessdesk.yCoord + 0.5D,
                                               this.chessdesk.zCoord + 0.5D) > 64.0D) {
