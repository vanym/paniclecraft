...
public class GuiButtonForChess1 extends GuiButton {
    ...
    public void drawButton(Minecraft par1Minecraft, int par2, int par3) {
        ...
        if (this.id == this.desk.lastFrom) {
            var5 = this.width * 4;
        }
        if (this.id == this.desk.lastTo) {
            var5 = this.width * 5;
        }
        if (this.id == this.desk.lastFrom && this.mode == 2) {
            var5 = this.width * 6;
        }
        if (this.id == this.desk.lastTo && this.mode == 2) {
            var5 = this.width * 7;
        }
        if (var4 && this.cbs) {
            var5 = this.width;
        }
        int var6 = Math.abs((int)this.desk.desk[this.id]);
        if (var6 > 6) {
            var6 -= 3;
        }
        this.drawTexturedModalRect(this.xPosition, this.yPosition, var5, 0, this.width,
                                   this.height);
        this.drawTexturedModalRect(this.xPosition, this.yPosition, var6 * this.width,
                                   (this.desk.desk[this.id] > 0 ? this.height : this.height * 2),
