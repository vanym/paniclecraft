...
public class PaintingFramePeripheral extends PicturePeripheral {
    ...
    protected final ThreadLocal<EnumFacing> pside;
    ...
    protected String getCurrentSide() {
        return Optional.ofNullable(this.pside.get()).map(EnumFacing::getName2).orElse("unknown");
    }
    
    @PeripheralMethod(33)
    protected void setSide(String name) throws LuaException, InterruptedException {
        try {
            this.pside.set(Arrays.stream(EnumFacing.VALUES)
