...
public class ChestCraftOverlayHandler implements IOverlayHandler{
	...
	public ChestCraftOverlayHandler(int x, int y){
		offsetx = x;
		offsety = y;
	}
	
	public ChestCraftOverlayHandler(){
		this(5, 11);
	}
	...
	public static boolean slotCheck(Slot slot){
		return (slot.inventory instanceof InventoryPlayer) || ((slot.inventory instanceof InventoryPrivateChest) && !(slot.inventory instanceof InventoryUpgradesPrivateChest));
	}
	...
	private void moveIngredients(GuiContainer gui, List<IngredientDistribution> assignedIngredients, int quantity){
		for(IngredientDistribution distrib : assignedIngredients){
			...
			for(Slot slot : (List<Slot>)gui.inventorySlots.inventorySlots){
				if(!slot.getHasStack() || !(slotCheck(slot)))
					continue;
	...
	private Slot[][] assignIngredSlots(GuiContainer gui, List<PositionedStack> ingredients, List<IngredientDistribution> assignedIngredients){
		Slot[][] recipeSlots = mapIngredSlots(gui, ingredients);// setup the
																// slot map
