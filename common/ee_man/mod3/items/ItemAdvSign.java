...
public class ItemAdvSign extends ItemMod3{
	...
	public ItemAdvSign(int par1, Block par2Post, Block par3Wall){
		super(par1);
		...
		if(par1ItemStack.hasTagCompound()){
			...
			if(tag.hasKey("SignText")){
				if(GuiScreen.isShiftKeyDown()){
					...
					String[] textAr = text.split(TileEntityAdvSign.separator, (text + '\u0000').split(TileEntityAdvSign.separator).length);
					for(int i = 0; i < textAr.length; i++)
						par3List.add(textAr[i]);
				}
				else
					par3List.add(Localization.get("text.pressShiftToSeeText"));
			}
		}
	}
		...
		if(!par2EntityPlayer.isSneaking()){
			...
			if(tile != null)
				if(tile instanceof TileEntitySign){
					...
					NBTTagCompound tag = par1ItemStack.getTagCompound();
					tag.setString("SignText", tileS.signText[0] + TileEntityAdvSign.separator + tileS.signText[1] + TileEntityAdvSign.separator + tileS.signText[2] + TileEntityAdvSign.separator + tileS.signText[3]);
		...
		if(par7 == 0){
			...
			if(!par3World.getBlockMaterial(par4, par5, par6).isSolid()){
				...
				if(!par2EntityPlayer.canPlayerEdit(par4, par5, par6, par7, par1ItemStack)){
					...
					if(!post.canPlaceBlockAt(par3World, par4, par5, par6)){
						return false;
					}
					else{
						if(par7 == 1){
							int var11 = MathHelper.floor_double((double)((par2EntityPlayer.rotationYaw + 180.0F) * 16.0F / 360.0F) + 0.5D) & 15;
							par3World.setBlock(par4, par5, par6, post.blockID, var11, 3);
						}
						else{
							par3World.setBlock(par4, par5, par6, wall.blockID, par7, 3);
						}
						
						--par1ItemStack.stackSize;
						TileEntityAdvSign var12 = (TileEntityAdvSign)par3World.getBlockTileEntity(par4, par5, par6);
